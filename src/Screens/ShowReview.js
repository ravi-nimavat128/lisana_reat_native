...
export class AddReview extends Component {
  ...
  _get_review = () => {
    ...
    axios
      .post('http://binarygeckos.com/lisana/api/get_job_review', formData, {
        headers: headers,
      })
      .then(Response => {
        if (Response.data.status == 1) {
          this.setState({
            isLoading: false,
            inq_title: Response.data.inquiry_name,
            img: Response.data.img,
            inq_date: Response.data.date,
            comment: Response.data.comment,
            rating: Response.data.rating_star,
          });
  ...
  _add_review = () => {
    ...
    let formData = new FormData();
    formData.append('user_id', this.state.quote_id);
    formData.append('inquiry_id', this.props.route.params.id);
    formData.append('rating_star', this.state.rating);
    formData.append('comment', this.state.comment);
        ...
        <ScrollView>
          <View style={{alignItems: 'center'}}>
            <Image
              ...
              source={{uri: this.state.img}}></Image>
            <Text
              style={{
                color: '#1F1F1F',
                fontSize: 20,
